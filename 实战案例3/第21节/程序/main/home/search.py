...
def 查询(查询参数):
    if '模块' in 查询参数:
        if 查询参数['模块'] == 1: #'销售对账'
            if '客户名称' in 查询参数:
                客户 = 查询参数['客户名称'].strip()
                精确搜索结果QuerySet = 销售入账记录.objects.filter(客户=客户).values('客户', '入账日期', '入账金额', '已到账款项', '剩余到账款项')
                if 精确搜索结果QuerySet is not None and len(精确搜索结果QuerySet) > 0:
                    精确搜索结果 = []
                    for item in 精确搜索结果QuerySet:
                        精确搜索结果.append({
                            '客户': item['客户'],
                            '入账日期': str(item['入账日期']),
                            '入账金额': item['入账金额'],
