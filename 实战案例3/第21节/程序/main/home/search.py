...
def 查询(查询参数):
    if '模块' in 查询参数:
        if 查询参数['模块'] == 1: #'销售对账'
            if '客户名称' in 查询参数:
                关键词str = 查询参数['客户名称'].strip()
                精确搜索结果by关键词 = 关键词.objects.filter(关键词=关键词str)
                if 精确搜索结果by关键词 is not None and len(精确搜索结果by关键词) > 0:
                    精确搜索结果 = 销售入账记录.objects.filter(客户=关键词str).values('客户', '入账日期', '入账金额', '已到账款项', '剩余到账款项')
