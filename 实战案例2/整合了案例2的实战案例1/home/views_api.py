...
def CNET新闻入库api(request):
    ...
    for item in json_obj:
        CNET新闻实例 = CNET新闻()
        CNET新闻实例.标题 = item['元数据']['标题']
        CNET新闻实例.标题中文翻译 = item['元数据']['标题_中文翻译']
        新闻发布日期_原始数据 = item['元数据']['创建日期']
        新闻发布日期=datetime(新闻发布日期_原始数据[0],新闻发布日期_原始数据[1],新闻发布日期_原始数据[2])
        CNET新闻实例.新闻发布日期 = 新闻发布日期
        CNET新闻实例.url = item['元数据']['url']
        CNET新闻实例.作者 = item['元数据']['作者']
        CNET新闻实例.权限 = "CNETNews"
        CNET新闻实例.新闻内容 = item['新闻内容']
        CNET新闻实例.摘要 = item['摘要']
        CNET新闻实例.新闻内容中文翻译 = item['新闻内容_中文翻译']
        CNET新闻实例.save()
